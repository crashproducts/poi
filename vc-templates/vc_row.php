...
$row_content_width = $anchor = $content_text_aligment = $enable_box_shadow = $simple_background_color = $simple_background_image = $background_image_position = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = '';
...
if ( $row_content_width !== 'grid' ) {
	...
	if ( $enable_box_shadow === 'yes' ) {
		$edgtf_vc_row_style[] = 'box-shadow: 0 -10px 27px 0 rgba(14, 8, 18, 0.61)';
	}
	...
	if ( $enable_box_shadow === 'yes' ) {
		$edgtf_grid_row_style[] = 'box-shadow: 0 -10px 27px 0 rgba(14, 8, 18, 0.61)';
	}
...
if ( $row_content_width === 'grid' ) {
	$edgtf_row_wrapper_start .= '<div class="edgtf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . overworld_edge_get_inline_style( $edgtf_grid_row_styles ) . '><div class="edgtf-row-grid-section">';
	$edgtf_row_wrapper_end   .= '</div></div>';
}

$edgtf_after_wrapper_open   = apply_filters('overworld_edge_filter_vc_after_wrapper_open', '', $atts);
$edgtf_before_wrapper_close = apply_filters('overworld_edge_filter_vc_before_wrapper_close', '', $atts);
$css_classes                = apply_filters('overworld_edge_filter_vc_css_classes', $css_classes, $atts);
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . overworld_edge_get_inline_style( $edgtf_vc_row_styles ) . '>';
$output .= $edgtf_after_wrapper_open;
$output .= wpb_js_remove_wpautop( $content );
$output .= $edgtf_before_wrapper_close;
$output .= '</div>';
$output .= $edgtf_row_wrapper_end;
$output .= $after_output;

echo overworld_edge_get_module_part( $output );
